...
eMail.addEventListener('input', function () {
    ...
    const errorMailReg0 = /^[0-9a-z_.-]+@[0-9a-z.-]+\.[a-z]{2,3}$/i;            // wrong email address
    const errorMailReg1 = /[,;"'}{!#$%&()\*\?\:\/\^\+\\\=\|]+/g;                // wrong characters
    const errorMailReg2 = /[A-Z]+/g;                                            // wrong uppercase
    ...
    else if (errorMailReg1.test(eMail.value)) {                                // input contains wrong characters
        divValidation[1].innerHTML = 'Incorrect characters ' + eMail.value.match(errorMailReg1);
        formValidation(eMail, 'fa-check-circle-o', 'fa-times-circle-o');
    } else if (errorMailReg2.test(eMail.value)) {                              // input contains uppercase
        divValidation[1].innerHTML = 'Use lowercases!';
        formValidation(eMail, 'fa-check-circle-o', 'fa-times-circle-o');
    } else if (!(errorMailReg0.test(eMail.value))) {                           // input contains wrong email address
